...
[D,I,N,U] = GetFileData(FILES,false,'Fly','Trial','HGain','WGain');
...
for kk = 1:N{1,end} % all trials
    ...
    load(filename,'FlyState','AI','rawTime') % load in fly kinematics & arena voltages
    
    vid.time            = rawTime;
    fly.time            = FlyState.Time;
...
%     pat.xpos            = pat.xpos * 3.75
    ...
    cla
    plot(fly.time,rad2deg(fly.head.pos))
    pause
    ...
    for jj = 1:N{1,4}
        for ii = 1:N{1,3}
            subplot(N{1,3},N{1,4},pp) ; hold on
                ...
                ylim([1 96])
                if any(pp==(1:N{1,4}))
                    title(['Wing Gain = ' num2str(U{1,4}{1}(ii))],'FontSize',12,'fontweight','bold')
                end
                if ((pp-1)/N{1,4}) == floor((pp-1)/N{1,4})
                    ylabel({['Head Gain = ' num2str(U{1,3}{1}(jj))],['Pattern Position(' char(176) ')']},...
    ...
    for jj = 1:N{1,4}
        for ii = 1:N{1,3}
            subplot(N{1,3},N{1,4},pp) ; hold on
                ...
                if ((pp-1)/N{1,4}) == floor((pp-1)/N{1,4})
                    ylabel({['Head Gain = ' num2str(U{1,3}{1}(jj))],['Head Position(' char(176) ')']},...
                        'FontSize',13,'fontweight','bold')
                else
                    yticks(0)
                    yticklabels('')  
    ...
    for jj = 1:N{1,4}
        for ii = 1:N{1,3}
            subplot(N{1,3},N{1,4},pp) ; hold on
                ...
                if ((pp-1)/N{1,4}) == floor((pp-1)/N{1,4})
                    ylabel({['Head Gain = ' num2str(U{1,3}{1}(jj))],['\Delta WBA(' char(176) ')']},...
                        'FontSize',13,'fontweight','bold')
                else
                    yticks(0)
                    yticklabels('')  
    ...
    for jj = 1:N.WGain
        for ii = 1:N.HGain
            subplot(N{1,3},N{1,4},pp) ; hold on
                ...
                %ylim(120*[-1 1])
                if any(pp==(1:N{1,4}))
                    title(['Wing Gain = ' num2str(U.WGain{1}(ii))],'FontSize',12,'fontweight','bold')
                end
                if ((pp-1)/N{1,4}) == floor((pp-1)/N{1,4})
                    ylabel(['Head Gain = ' num2str(U.HGain{1}(jj))],...
                        'FontSize',13,'fontweight','bold')
                else
                    yticks(0)
                    yticklabels('')  
    ...
    for jj = 1:N{1,4}
        for ii = 1:N{1,3}
            subplot(N{1,3},N{1,4},pp) ; hold on
                ...
                if ((pp-1)/N{1,4}) == floor((pp-1)/N{1,4})
                    ylabel(['Head Gain = ' num2str(U{1,3}{1}(jj))],...
                        'FontSize',13,'fontweight','bold')
                else
                    yticks(0)
                    yticklabels('')  
